...
class ParcoursSuiviController extends ApiController
{
    ...
    /**
     * @OA\Get(
     *      path="/api/anatt-candidat/parcours-suivis/all",
    ...
    /**
     * @OA\Post(
     *      path="/api/anatt-candidat/parcours-suivis/absences",
     ...
     *                  ),
     *                  @OA\Property(
     *                      property="candidat_justif_absence_id",
     ...
     * )
     */
    public function storeJustif(Request $request)
    {
        $v = Validator::make($request->all(), [
            'candidat_justif_absence_id' => 'required|exists:candidat_justif_absences,id',
        ...
        try {
            $candidat_justif_absence_id = $request->input('candidat_justif_absence_id');
            $agent_id = $request->input('agent_id');
            $state = $request->input('state');
            // Vérifier si le dossier candidat existe avec l'ID donné
            $candidatJustif = CandidatJustifAbsence::findOrFail($candidat_justif_absence_id);
            if (!$candidatJustif) {
                return  $this->errorResponse("Justification introuvable");
            }
            $npi = $candidatJustif->npi;
            $user = User::where('npi', $npi)->firstOrFail();

            $candidat_id = $user->id;
            $categorie_permis_id = $candidatJustif->categorie_permis_id;
            $dossier_candidat_id = $candidatJustif->dossier_candidat_id;
            $dossier_session_id = $candidatJustif->dossier_session_id;
            $date_soumission = now();
            ...
            if ($state === 'rejet') {
                ...
                $parcoursSuivi->slug = "validation-justif-failed";
                ...
                $parcoursSuivi->message = "Votre tentative de justification a été rejeté. Consignes à suivre :" . $consignes . " Voulez-vous poursuivre avec ce dossier ?";
                ...
                $parcoursSuivi->candidat_justif_absence_id = $candidat_justif_absence_id;
                ...
                $parcoursSuivi = new ParcoursSuivi();
                $parcoursSuivi->npi = $npi;
                $parcoursSuivi->slug = "validation-justif-success";
                $parcoursSuivi->service = 'Permis';
                $parcoursSuivi->candidat_id = $candidat_id;
                $parcoursSuivi->dossier_candidat_id = $dossier_candidat_id;
                $parcoursSuivi->categorie_permis_id = $categorie_permis_id;
                $parcoursSuivi->message = "L'ANaTT vient d'analyser et de valider votre justification, vous pouvez à présent choisir votre prochaine session";
                $parcoursSuivi->agent_id = $agent_id;
                $parcoursSuivi->bouton = json_encode(['bouton' => 'Session', 'status' => '1']);
                $parcoursSuivi->dossier_session_id = $dossier_session_id;
                $parcoursSuivi->candidat_justif_absence_id = $candidat_justif_absence_id;
                $parcoursSuivi->date_action = $date_soumission;
                $parcoursSuivi->save();
            ...
            return $this->successResponse($parcoursSuivi, 'Traitement de justification du candidat effectué avec succès.');
        } catch (\Illuminate\Database\Eloquent\ModelNotFoundException $e) {
            logger()->error($e);
            return $this->errorResponse('Dossier candidat non trouvé.', null, null, 422);
        } catch (\Throwable $e) {
            logger()->error($e);
            return $this->errorResponse('Une erreur est survenue lors de l\'inssertion', null, null, 500);
        }
    }

    /**
     * @OA\Post(
     *      path="/api/anatt-candidat/parcours-suivis/convocation-codes",
    ...
    public function storeConvocationCode(Request $request)
    {
        ...
        try {
            ...
            foreach ($dossierSessions as $dossierSession) {
                ...
                $existingEntry = ParcoursSuivi::where('dossier_candidat_id', $dossier_candidat_id)
