...
use App\Models\AutoEcole\InscriptionCandidat;
...
class AutoEcoleController extends DataController
{
    ...
    public function index(Request $request)
    {
        ...
        $autoes = AutoEcole::with(['commune', 'departement']);
        ...
        try {
            ...
            # Sélectionne les auto-écoles pour l'actuel candidat connecté
            $user = Auth::user();
            $aAffilies = InscriptionCandidat::where("npi", $user->npi)->get()->pluck("auto_ecole_id")->all();
            $autoes = $autoes->whereIn("id", $aAffilies);

            // Ajoutez cette clause pour filtrer les auto-écoles avec un statut vrai
            $autoes = $autoes->where('status', true);
            /************************** Transformation des suivis ************************************ */
            # Map chaque ligne
            $transforAutoe = function (AutoEcole $autoEcole) {
                return $autoEcole;
            };
