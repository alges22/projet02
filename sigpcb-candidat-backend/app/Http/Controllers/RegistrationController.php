...
class RegistrationController extends ApiController
{
    ...
    public function verifyNpi(Request $request)
    {
        try {
            $validator = Validator::make($request->all(), [
                'npi' => 'required|string|max:10|exists:candidats,npi',
            ], [
    ...
    public function register(Request $request)
    {
        try {
            $validator = Validator::make($request->all(), [
                'npi' => 'required|string|max:10|unique:candidats,npi',
            ], [
                'npi.integer' => 'Le numéro NPI doit être un entier',
            ...
            $user_phone = data_get($candidat, 'telephone');

            $country_code = '229';
            $text = 'Votre code de connexion est : ' . $otp_code;

            # Enregistrement du sexe
            $anipInfos = GetCandidat::findOne($npi);

            if (!$anipInfos) {
                return $this->errorResponse("Votre NPI n'est pas retrouvé chez ANIP");
            }

            $sexe = $anipInfos['sexe'] ?? null;
            ...
            if (Sms::sendSMS($country_code, $user_phone, $text)) {
                // Vérifier si l'email est présent et non nul
                if ($request->has('email') && !empty($request->input('email'))) {
                    $email = $request->input('email');
                    // Vérifier si l'email a le format correct
                    if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
    ...
    public function verifyOtp(Request $request)
    {
        try {
            ...
            if (Auth::loginUsingId($request->user_id)) {
                ...
                $latestAction = UserAction::where('candidat_id', $user->id)->latest()->first();
                $url = $request->fullUrl();
                if ($latestAction) {
    ...
    public function resendCode(Request $request)
    {
        try {
            ...
            $user_phone = $anipInfos['telephone'] ?? null;
