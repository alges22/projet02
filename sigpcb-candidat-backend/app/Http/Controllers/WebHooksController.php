...
class WebHooksController extends Controller
{
    public function fedapay()
    {
        ...
        try {
            ...
            $userTransaction = $this->findTransaction($data['transactionId']);
            if (!$userTransaction) {
                return;
            }
        ...
        switch ($event->name) {
            case 'transaction.approved':
                $this->transactionApproved($userTransaction);
                break;
            case 'transaction.canceled':
                $this->transactionCanceled($userTransaction);
                break;
            default:
                http_response_code(400);
                exit;
    ...
    private function transactionApproved(UserTransaction $transaction)
    {
        ...
        switch ($transaction->service) {
            case Service::DEMANDE_AUTHENTICITE:
                app(AuthenticiteController::class)->procced($request);
                break;
            case Service::DEMANDE_PERMIS_CONDUIRE:
                app(CandidatPaymentController::class)->procced($request);
                break;
            default:
                # code...
                break;
    ...
    private function transactionCanceled(UserTransaction $transaction)
    {
    ...
    {
        return UserTransaction::where('transaction_id', $id)->first();
    }
}
