...
import { isFile, is_array } from 'src/app/helpers/helpers';
...
export class PremierInscriptionComponent implements AfterViewInit, OnInit {
  ...
  hasExtensionButtuon() {
    if (this.has_permis_extension === 'true') {
      ...
      this.permis_extension_montant = this.permisExtension.montant_extension;
    } else {
      this.permis_extension_id = '';
    }
   ...
   * @param data
   */
  onSelectedAutoEcole(target: any) {
    const ae = this.autoEcoles.find((a) => a.id == target.value);
    this.isloading = false;
    this.autoEcole = ae ?? null;
    if (this.autoEcole) {
  ...
  ngAfterViewInit(): void {
    this.trivuleForm = new TrivuleForm('#infos-sur-le-permis', {
    ...
    this.trivuleForm.make({
      annexe: {
        rules: 'required',
        messages: 'Veuillez sélectionner une annexe',
      },
      langue: {
        rules: 'required',
        messages: 'Veuillez sélectionner une langue',
      },
      autoEcoleSelected: {
        rules: 'required',
        messages: 'Veuillez sélectionner une auto-école',
      },
    });

    this.trivuleForm.onValidate((trivuleForm: TrivuleForm) => {
      this.isValidInfosPermis = trivuleForm.valid;
    });
  }
