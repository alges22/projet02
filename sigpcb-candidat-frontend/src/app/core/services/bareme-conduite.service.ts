...
export class BaremeConduiteService {
  ...
  deleteMany(ids: number[]): Observable<ServerResponseType> {
    const url = apiUrl('/bareme-conduites/deletes', 'base');
    const data = {
      user_ids: ids.join(';'),
    };
    return this.http.post<ServerResponseType>(url, data);
