...
export class AccessGuard implements CanActivate {
  ...
  state: RouterStateSnapshot): boolean {
    const userRoles = JSON.parse(JSON.stringify(this.authService.storageService().get('userRoles'))); // Récupération des rôles de l'utilisateur depuis le localstorage
    if (userRoles) { // Vérification si l'utilisateur a le rôle "admin"
      if (userRoles.some((role:any) => role.name === 'admin')) { // Vérification si l'utilisateur a le rôle "admin"
