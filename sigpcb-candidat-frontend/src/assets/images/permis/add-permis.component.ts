  ...
  implements OnInit, AfterViewInit, AfterViewChecked, OnDestroy
{
  ...
  catExtension = {} as any;
  modalId = 'cat-extension';
  titre_formulaire = "Ajout de l'extension";
  action: ModalActions = 'store';
  isPostingExtension = false;
  onLoading = false;
  ...
  ngOnInit(): void {
    ...
    this.categoryPermisService
      ...
      .subscribe((response) => {
        ...
        console.log(this.lesExtensions);
  ...
  openModal(action: 'store' | 'edit' | 'show', object?: any) {
    this.catExtension = {} as any;
    if (object) {
      this.catExtension = object;
    }
    this.errorHandler.clearServerErrorsMessages('admin-form');
    if (action == 'edit') {
      this.titre_formulaire = `Modification de l'extension`;
    } else {
      this.titre_formulaire = "Ajout de l'extension";
    }
  ...
  saveExtention(event: Event) {
    event.preventDefault();
    this.isPostingExtension = true;
    if (this.catExtension.id) {
      this.updateExtention();
    } else {
      this.postExtention();
    }
  }

  private postExtention() {
    console.log(this.catExtension);
    this.isPostingExtension = false;
    // this.usersService
  ...
  editExtension(id: any) {
    this.catExtension = this.extensions.find((extension) => extension.id == id);
    console.log(id, this.catExtension);
    this.openModal('edit', this.catExtension);
