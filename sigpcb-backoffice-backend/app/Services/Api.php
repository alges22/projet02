...
class Api
{
    ...
    public static function anip(string $method, string $path, array $data = [])
    {
        ...
        $headers = Atk::anattToken();

        if (strtolower($method) == 'post') {
            return Http::timeout(self::TIMEOUT_API)->withHeaders($headers)->post($endpoint . $path, $data);
        } elseif (strtolower($method) == 'delete') {
            return Http::timeout(self::TIMEOUT_API)->withHeaders($headers)->delete($endpoint . $path, $data);
        } elseif (strtolower($method) == 'put') {
            return Http::timeout(self::TIMEOUT_API)->withHeaders($headers)->put($endpoint . $path, $data);
        } else {
            return Http::timeout(self::TIMEOUT_API)->withHeaders($headers)->get($endpoint . $path, $data);
        }
    }
    ...
    public static function compo(string $method, string $path, array $data = [])
    {
        $endpoint = env('COMPO_ENDPOINT');
        $headers = Atk::anattToken();
