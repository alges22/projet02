...
use App\Models\Candidat\AuthenticiteRejet;
...
class AuthenticiteController extends ApiController
{
    ...
    {
        $this->hasAnyPermission(["read-permit-authentication", "edit-permit-authentication", "all"]);
        $query = Authenticite::orderByDesc('id');

        // Appliquer les filtres
        $query = $this->applyFilters($query);

        $demandes = $query->paginate(25);
    ...
    {
        $this->hasAnyPermission(["edit-permit-authentication", "all"]);

        $validator = Validator::make($request->all(), [
            'authenticite_id' => 'required|integer|min:1',
        ...
        try {
            ...
            $da = Authenticite::findOrFail($request->authenticite_id);
            ...
            $parcoursSuiviData = [
                'npi' => $npi,
                'slug' => 'demande-authenticite-validate',
                'service' => 'Authenticite',
                'candidat_id' => $candidat_id,
                'message' => 'Votre demande d\'authenticité a été validée avec succès. Consigne :' . $consigne,
                'eservice' => '{"Model":"Authenticite","id":"' . $da->id . '"}',
                'date_action' => now(),
            ];
    ...
    public function rejectDemande(Request $request)
    {
        ...
        try {
            ...
            try {
                ...
                $AuthenticiteRejet = new AuthenticiteRejet();
                $AuthenticiteRejet->authenticite_id = $da->id;
                $AuthenticiteRejet->motif = $request->consigne;
                $AuthenticiteRejet->state = 'init';
                $AuthenticiteRejet->save();

                //notifier le candidat
                $parcoursSuiviData = [
                    'npi' => $npi,
                    'slug' => 'demande-authenticite-rejected',
                    'service' => 'Authenticite',
                    'candidat_id' => $candidat_id,
                    'message' => 'Votre demande d\'authenticité a été rejetée. Consigne : ' . $request->consigne,
                    'eservice' => '{"Model":"AuthenticiteRejet","id":"' . $AuthenticiteRejet->id . '"}',
                    'date_action' => now(),
                ];
