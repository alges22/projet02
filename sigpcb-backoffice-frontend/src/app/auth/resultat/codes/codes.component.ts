...
export class CodesComponent {
  ...
  fetch(filter: string) {
    ...
    for (const resultat of resultats) {
      ...
      if (resultat.resultat_code == 'success') {
        resultat.status = 'Admis(e)';
      }
      if (resultat.resultat_code == 'failed') {
        resultat.status = 'Récalé(e)';
      }
      if (resultat.presence == 'abscent') {
  ...
  private filterResults(resultats: any[], type = 'admis') {
    if (type == 'admis') {
      return resultats.filter((resultat) => {
        return resultat.resultat_code == 'success';
      });
    } else if (type == 'recales') {
      return resultats.filter((resultat) => {
        return resultat.resultat_code == 'failed';
      });
    } else if (type == 'abscent') {
      return resultats.filter((resultat) => {
        return resultat.presence == 'abscent';
      });
    } else {
      return resultats;
    }
