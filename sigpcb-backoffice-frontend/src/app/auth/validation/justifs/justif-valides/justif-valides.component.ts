...
@Component({
  selector: 'app-justif-valides',
  templateUrl: './justif-valides.component.html',
  styleUrls: ['./justif-valides.component.scss'],
})
export class JustifValidesComponent {
  ...
  onValidate(event: any, index: number): void {
    if (event.state === 'validate') {
      ...
      this.rejectData.justifId = event.justifId;
  ...
  reject(): void {
    ...
    this.validationService
      .reject(this.rejectData.justifId, {
