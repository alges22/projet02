...
@Component({
  selector: 'app-demande-examinateur',
  templateUrl: './demande-examinateur.component.html',
  styleUrls: ['./demande-examinateur.component.scss'],
})
export class DemandeExaminateurComponent {
  pageNumber = 1;
  paginate_data: any = {};
  ready = true;
  ...
  onValidate(event: { data: Examinateur; state: string }, index: number): void {
    ...
    if (event.state === 'validate') {
      ...
      this.rejetData.title = `Rejet de demande pour devenir examinateur de <span class="text-uppercase">  ${event.data.demandeur_info.nom} ${event.data.demandeur_info.prenoms}</span>`;
      this.rejetData.demandeId = event.data.id;
  ...
  reject(): void {
    ...
    this.recrutementExaminateurService
      .reject(this.rejetData.demandeId, {
        motif: '',
        consignes: this.rejetData.consigne,
      })
      .pipe(this.errorHandler.handleServerErrors())
      .subscribe((response) => {
